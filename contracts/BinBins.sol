...
function getRentedBinBin (address walletAddress) external view returns (uint id){
    require(isUser(walletAddress), "User does not exist");
    return users[walletAddress].rentedBinBinId;
}
...
function StartTime (address walletAddress) external view onlyOwner returns(uint){
 require(isUser(walletAddress), "User does not exist");
    return users[walletAddress].startTime;
}
}
